{
  flake.modules.homeManager.waybar.imports = [
    (
      { pkgs, lib, ... }:
      let
        # Wallpaper list — to add/remove wallpapers, edit this list. Schedules
        # auto-adjust. Use `nix-prefetch-url --name <clean-name> <url>` to get
        # sha256 for new entries. The `name` field must be a valid Nix store
        ...
        # Night + optionally blurred version for the overview backdrop.
